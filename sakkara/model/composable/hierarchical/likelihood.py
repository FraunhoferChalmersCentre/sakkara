...
class Likelihood(RandomVariable, ABC):
    def __init__(self, generator: Callable, obs_data: SeriesComponent, name='likelihood', columns='obs',
                 nan_var_mask: Union[Dict[str, Any], Any] = 0.,
                 nan_data_mask: Union[Dict[str, Any], Any] = 1e-15, **kwargs: Any):
        ...
        if np.any(np.isnan(obs_data.values)):
            ...
            for k, v in kwargs.items():
                ...
                non_nandata = np.array([v if not np.isnan(v) else nan_data_mask for v in obs_data.values])

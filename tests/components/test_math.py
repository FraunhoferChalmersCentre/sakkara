...
def variables():
    ...
    c = DataComponent(5, group='global')

    return {2: a, 3: b, 5: c, 7: 7}


def get_variable(k1, k2, x):
    if 5 <= k1 and 5 <= k2:
        return x.variable[0]
    else:
        return pm.draw(x.variable).flatten()[0]
    ...
    for k1, v1 in variables.items():
        for k2, v2 in variables.items():
            ...
            with build(simple_df, x):
                assert get_variable(k1, k2, x) == pytest.approx(op(k1, k2))

            x = op(v2, v1)
            with build(simple_df, x):
                assert get_variable(k1, k2, x) == pytest.approx(op(k2, k1))
